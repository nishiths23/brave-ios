...
public class DebouncingResourceDownloader {
  ...
  struct MatcherRule: Decodable {
    ...
    let preferences: Set<Preferance>
    ...
    /// Actions in a strictly typed format and split up into an array
    /// - Note: Unrecognized actions will be filtered out
    private static func makePreferences(fromString string: String?) throws -> Set<Preferance> {
      guard let prefStrings = string?.split(separator: ",") else {
        return []
      }
      
      return Set(try prefStrings.map({
        if let action = Preferance(rawValue: String($0)) {
          return action
        } else {
          throw RuleError.unsupportedPref(String($0))
        }
      }))
    }
  }
  ...
  public struct RedirectRule {
    ...
    let preferences: Set<MatcherRule.Preferance>
    let prependScheme: String?
  ...
  struct Matcher {
    ...
    init(rules: [Result<MatcherRule, Error>]) {
      ...
      for result in rules {
        ...
        } catch RuleError.unsupportedPref(let rawRule) {
